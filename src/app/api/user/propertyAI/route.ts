...
import { getServerCookieValue } from "@/function/cookies";
...
export async function POST(request: Request) {
  try {
    ...
    const {mode , answers , questions} = await request.json();
    ...
    const externalResponse = await axios.post(
      `${EXTERNAL_BASE_URL}/api/ai/genrate`,
      {mode ,answers , questions}, // Send the ID in the body to the external API
      {
        headers: {
